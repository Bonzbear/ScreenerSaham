...
# contoh ticker (bisa diganti 900 saham BEI)
    ...
    for i in range(200, len(df)-1):
        ...
        if setup == "V1":
            ...
            if (
                ...
                float(prev["Close"]) < close and
                close > float(today["SMA5"]) and
                value > 5000000000
            ):
                signal = True
        ...
        if setup == "MA50":
            ...
            if (
                llv5 > float(today["SMA50"]) and
                close >= float(today["SMA50"])*0.99 and
                close <= float(today["SMA50"])*1.02 and
                value > 1000000000
            ):
                signal = True
        ...
        if setup == "MA200":
            ...
            if (
                llv5 > float(today["SMA200"]) and
                close >= float(today["SMA200"])*0.99 and
                close <= float(today["SMA200"])*1.02 and
                value > 1000000000
            ):
                signal = True
        ...
        if setup == "BBMid":
            ...
            if (
                close >= float(today["BB_MID"])*0.98 and
                close <= float(today["BB_MID"])*1.02 and
                value > 1000000000 and
                float(today["EMA10"]) > float(today["EMA20"]) and
                float(today["EMA20"]) > float(today["EMA50"]) and
                float(today["BBW"]) >= 0.1 and
                close > float(today["BB_MID"])
            ):
                signal = True
        ...
        if setup == "BBRev":
            ...
            if (
                float(prev["Low"]) < float(prev["BB_LOW"]) and
                float(prev["Close"]) < float(prev["BB_LOW"]) and
                close > float(today["BB_LOW"]) and
                value > 1000000000 and
                volume > prev_volume and
                float(prev["High"]) < float(today["High"]) and
        ...
        gain = (float(nextday["High"]) - close) / close * 100
