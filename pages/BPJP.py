...
def get_data(ticker):
    ...
    df = df.dropna(subset=["SMA5"])

    return df

# =========================
# LAYER 1 (SCREENER KAMU)
# =========================
def screener_layer1(df):
    today = df.iloc[-1]
    prev = df.iloc[-2]
...
def analyze_distribution(df):
    ...
    for i in range(1, len(df)-1):
        ...
        gain = (next_day["High"] - entry) / entry * 100
        gains.append(gain)
...
def get_stats(gains):
    ...
    for lvl in TP_LEVELS:
        count = sum(1 for g in gains if g >= lvl)
        stats[lvl] = round((count / total) * 100, 2) if total > 0 else 0
...
if st.button("Run Screener"):
    ...
    for i, ticker in enumerate(tickers):
        ...
        # Layer 1
        if screener_layer1(df):
            layer1_pass.append(ticker)

            # Layer 2 (PINE)
            if screener_layer2(df):
    ...
    # OUTPUT SCREENER
    # =========================
    st.write("## 🔍 Layer 1 (Screener Kamu)")
    st.write(layer1_pass)

    st.write("## 🔥 Final (Setelah Filter Pine)")
    if len(final_selected) == 0:
        st.warning("Tidak ada saham yang lolos layer 2")
    else:
        st.success(f"{len(final_selected)} saham lolos final")
        # =========================
        # ANALISA PROBABILITY
        # =========================
        ...
        for ticker in final_selected:
            ...
            df = get_data(ticker)
            gains = analyze_distribution(df)

            stats, total_trade = get_stats(gains)
        ...
        if not result_df.empty:
            result_df = result_df.sort_values(by="TP 1.5%", ascending=False)
