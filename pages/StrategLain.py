...
# SIGNALS
# =========================
def signal_ma50(df, i):
    prev_llv5 = df["Low"].rolling(5).min().iloc[i-1]
    return (
        prev_llv5 > df["SMA50"].iloc[i-1] and
        df["Close"].iloc[i] >= df["SMA50"].iloc[i]*0.99 and
        df["Close"].iloc[i] <= df["SMA50"].iloc[i]*1.02 and
        df["Value"].iloc[i] > 1_000_000_000
    )

def signal_ma200(df, i):
    prev_llv5 = df["Low"].rolling(5).min().iloc[i-1]
    return (
        prev_llv5 > df["SMA200"].iloc[i-1] and
        df["Close"].iloc[i] >= df["SMA200"].iloc[i]*0.99 and
        df["Close"].iloc[i] <= df["SMA200"].iloc[i]*1.02 and
        df["Value"].iloc[i] > 1_000_000_000
    )

def signal_bbmid(df, i):
    return (
        df["Close"].iloc[i] >= df["BB_MID"].iloc[i]*0.98 and
        df["Close"].iloc[i] <= df["BB_MID"].iloc[i]*1.02 and
        df["Value"].iloc[i] > 1_000_000_000 and
        df["EMA10"].iloc[i] > df["EMA20"].iloc[i] > df["EMA50"].iloc[i] and
        df["BB_WIDTH"].iloc[i] >= 0.1 and
        df["Close"].iloc[i] > df["BB_MID"].iloc[i]
    )

def signal_v12(df, i):
    return (
        df["Close"].iloc[i] > df["SMA5"].iloc[i] and
        df["Close"].iloc[i-1] > df["SMA5"].iloc[i-1] and
        df["Close"].iloc[i-2] > df["SMA5"].iloc[i-2] and
        (df["High"].iloc[i-2] / df["Close"].iloc[i-3] >= 1.1) and
...
def extract_features(df, i):
    ...
    return {
        "dist_sma50": abs(df["Close"].iloc[i] - df["SMA50"].iloc[i]) / df["SMA50"].iloc[i],
        "value": df["Value"].iloc[i],
        "bb_width": df["BB_WIDTH"].iloc[i],
        "ema_trend": df["EMA10"].iloc[i] - df["EMA20"].iloc[i]
    }

# =========================
# BACKTEST + FEATURE LOG
# =========================
def backtest_with_features(df, signal_func):
...
# =========================
# SETUP SCORE (DATA-DRIVEN)
# =========================
def compute_setup_score(current_feat, hist_df):
    ...
    for col in ["dist_sma50","value","bb_width","ema_trend"]:
        ...
        percentile = (series < current_feat[col]).mean()
...
# SCREENER
# =========================
def run_screener(data, strategy):
    ...
    for ticker in TICKERS:
        ...
        if not func(df, len(df)-1):
            continue

        hist = backtest_with_features(df, func)

        if hist.empty:
            continue

        winrate = (hist["return"] >= 0.015).mean() * 100
        ev = hist["return"].mean() * 100

        current_feat = extract_features(df, len(df)-1)
        setup = compute_setup_score(current_feat, hist)

        probability = (winrate * 0.6) + (setup * 0.4)
...
# UI
# =========================
st.title("Quant Screener (Data-Driven Setup Score)")

strategy = st.selectbox(
    "Pilih Strategi",
    ["MA50","MA200","BBMID","V1.2"]
)
