...
st.title("Screener Saham Indonesia (Fast Version)")
...
def backtest_fast(df):
    ...
    winrate_v1 = (win_v1/total_v1*100) if total_v1 > 0 else None
    winrate_v2 = (win_v2/total_v2*100) if total_v2 > 0 else None

    return winrate_v1, winrate_v2
...
def calculate_score(df):
    ...
    if prev_close < prev_ma5: score += 125
    if vol > vol_ma20: score += 125
    if vol > vol_ma5: score += 125
    if low > prev_low: score += 125
    if high > prev_high: score += 125
    if (open_price - low) > (high - close): score += 125
    if close > vwap: score += 125
    if prev_close < prev_vwap: score += 125
...
def run_screener():
    ...
    for i,ticker in enumerate(TICKERS):
        ...
        # hitung sekali saja
        winrate_v1, winrate_v2 = backtest_fast(df)

        winrates = []

        if "V1" in signals and winrate_v1:
            winrates.append(winrate_v1)

        if "V2" in signals and winrate_v2:
            winrates.append(winrate_v2)

        winrate = round(sum(winrates)/len(winrates),2) if winrates else 0
