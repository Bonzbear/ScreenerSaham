...
TP_LEVELS = [1, 1.5, 2, 3, 5, 10]  # persen

st.title("Screener + Probability Engine (Adaptasi Pine Script)")
...
# =========================
# SCREENER HARI TERAKHIR
# =========================
def is_today_signal(df):
    if len(df) < 2:
        return False
    ...
    if today["Close"] < prev["Close"] * (1 + MIN_JUMP):
        return False
...
if st.button("Run Screener & Analysis"):
    ...
    for i, ticker in enumerate(tickers):
        ...
        if is_today_signal(df):
            selected.append(ticker)

    progress.empty()
    status.empty()

    st.write("## 🔍 Hasil Screener Hari Ini")

    if len(selected) == 0:
        st.warning("Tidak ada saham yang memenuhi kriteria hari ini")
    else:
        st.success(f"{len(selected)} saham lolos")
        st.write(selected)
        ...
        if not result_df.empty:
            ...
            st.warning("Tidak ada data historis yang valid")
