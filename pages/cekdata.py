...
print("Jumlah ticker:",len(tickers))
...
for ticker in tickers:
    ...
    try:
        ...
        if volume > last["VOL_MA20"]:
            score+=125

        if volume > last["VOL_MA5"]:
            score+=125
        ...
        if (last["Open"]-last["Low"]) > (last["High"]-last["Close"]):
            score+=125

        if close > last["VWAP"]:
            score+=125

        if prev_close < prev["VWAP"]:
            score+=125
