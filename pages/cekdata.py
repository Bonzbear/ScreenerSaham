...
def format_telegram(df):
    ...
    for i, row in df.head(5).iterrows():
        ...
        warn = row.get("Warning", "")

        msg += f"<b>{i+1}. {ticker} {warn}</b>\n"

    return msg


# =========================
# DATA FETCH (SAFE PER TICKER)
# =========================
...
def calculate_score(df):
    ...
    open_ = scalar(today["Open"])
    high = scalar(today["High"])
    low = scalar(today["Low"])
    close = scalar(today["Close"])

    score = 0
    warning = ""

    if scalar(prev["Close"]) < scalar(prev["SMA5"]):
        score += 125

    if scalar(today["Volume"]) > scalar(today["VOLMA20"]):
        score += 125

    if scalar(today["Volume"]) > scalar(today["VOLMA5"]):
        score += 125

    if low > scalar(prev["Low"]):
        score += 125

    if high > scalar(prev["High"]):
        score += 125

    if (open_ - low) > (high - close):
        score += 125

    if close > scalar(today["VWAP"]):
        score += 125

    if scalar(prev["Close"]) < scalar(prev["VWAP"]):
        score += 125
