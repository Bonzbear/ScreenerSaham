...
# SETUP SCORE
# =========================
def setup_score(df, strategy):

    today = df.iloc[-1]
    score = 0

    if strategy in ["MA50","MA200"]:
        if abs(today["Close"] - today["SMA50"]) / today["SMA50"] < 0.01:
            score += 50
        if today["Value"] > 2_000_000_000:
            score += 50

    elif strategy == "BBMID":
        if today["BB_WIDTH"] > 0.15:
            score += 50
        if today["EMA10"] > today["EMA20"]:
            score += 50

    elif strategy == "V1.2":
        score += 50  # base pattern
        if today["Value"] > 2_000_000_000:
            score += 50
...
def run_screener(data, strategy):
    ...
    for ticker in TICKERS:
        ...
        winrate, ev = backtest(df, func)
        setup = setup_score(df, strategy)
...
# UI
# =========================
st.title("Screener + Backtest (2 Tahun)")
