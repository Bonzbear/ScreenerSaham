...
def run_screener():
    ...
    for i,ticker in enumerate(TICKERS):
        ...
        prev_ma5 = df["SMA5"].iloc[-2]

        signals = []

        if volume > df["Volume"].iloc[-2] and prev_close < close and close > sma5 and value > 5000000000:
            signals.append("V1")

        if llv5 > sma50 and close >= sma50*0.99 and close <= sma50*1.02 and value > 1000000000:
            signals.append("MA50")

        if llv5 > sma200 and close >= sma200*0.99 and close <= sma200*1.02 and value > 1000000000:
            signals.append("MA200")

        if close >= bb_mid*0.98 and close <= bb_mid*1.02 and value > 1000000000 and ema10 > ema20 and ema20 > ema50 and bbw >= 0.1 and close > bb_mid:
            signals.append("BBMid")

        if prev_low < prev_bb_low and prev_close < prev_bb_low and close > bb_low and value > 1000000000 and volume > df["Volume"].iloc[-2] and prev_high < high and close > prev_close:
    ...
    if not df.empty:
        ...
        df["Winrate_TP1.5%"] = df["Winrate_TP1.5%"].fillna(0)

        df["Probability_Up_%"] = (
            (df["Score"] / 8 * 0.6) +
            (df["Winrate_TP1.5%"] / 100 * 0.4)
        ) * 100
