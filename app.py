...
def backtest_setup(df, setup):
    ...
    for i in range(200, len(df)-1):
        ...
        if setup == "MA50":
            ...
            signal = (
                llv5 > today["SMA50"] and
                close >= today["SMA50"]*0.99 and
                close <= today["SMA50"]*1.02 and
                value > 1000000000
            )
        ...
        if setup == "MA200":
            ...
            signal = (
                llv5 > today["SMA200"] and
                close >= today["SMA200"]*0.99 and
                close <= today["SMA200"]*1.02 and
                value > 1000000000
            )
        ...
        if setup == "BBMid":
            ...
            signal = (
                close >= today["BB_MID"]*0.98 and
                close <= today["BB_MID"]*1.02 and
                value > 1000000000 and
                today["EMA10"] > today["EMA20"] and
                today["EMA20"] > today["EMA50"] and
                today["BBW"] >= 0.1 and
                close > today["BB_MID"]
            )
        ...
        if setup == "BBRev":
            ...
            signal = (
                prev["Low"] < prev["BB_LOW"] and
                prev["Close"] < prev["BB_LOW"] and
                close > today["BB_LOW"] and
                value > 1000000000 and
                volume > prev_volume and
                prev["High"] < today["High"] and
                close > prev["Close"]
            )
        ...
        gain = (nextday["High"] - close) / close * 100
...
def run_screener():
    ...
    for i,ticker in enumerate(TICKERS):
        ...
        if volume > prev["Volume"] and prev["Close"] < close and close > today["SMA5"] and value > 5000000000:
            signals.append("V1")

        llv5 = df["Low"].iloc[-6:-1].min()

        if llv5 > today["SMA50"] and close >= today["SMA50"]*0.99 and close <= today["SMA50"]*1.02 and value > 1000000000:
            signals.append("MA50")

        if llv5 > today["SMA200"] and close >= today["SMA200"]*0.99 and close <= today["SMA200"]*1.02 and value > 1000000000:
            signals.append("MA200")

        if close >= today["BB_MID"]*0.98 and close <= today["BB_MID"]*1.02 and value > 1000000000 and today["EMA10"] > today["EMA20"] and today["EMA20"] > today["EMA50"] and today["BBW"] >= 0.1 and close > today["BB_MID"]:
            signals.append("BBMid")

        if prev["Low"] < prev["BB_LOW"] and prev["Close"] < prev["BB_LOW"] and close > today["BB_LOW"] and value > 1000000000 and volume > prev["Volume"] and prev["High"] < today["High"] and close > prev["Close"]:
        ...
        winrate = round(sum(winrates)/len(winrates),2) if len(winrates) else None
